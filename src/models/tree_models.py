...
class BaseTreeModel(ABC):
    ...
    def fit(
        ...
        **kwargs
        ...
        Args:
            sample_weight: Per-observation training weights. Use for oversampled
                           datasets (inverse sampling probability) or reject-inference
                           augmentation (augmented population weights).
        """
        pass
    ...
    def fit(
        ...
        **kwargs
        ...
        Args:
            ...
            sample_weight:  Per-observation weights (inverse sampling probability
                            for oversampled datasets or reject-inference augmentation)
        ...
        logger.info(f"Training LightGBM model (sample_weight={'yes' if sample_weight is not None else 'no'})...")
...
class XGBoostModel(BaseTreeModel):
    ...
    def fit(
        ...
        **kwargs
        ...
        Args:
            sample_weight: Per-observation weights passed to XGBClassifier/Regressor fit.
        """
        ...
        logger.info(f"Training XGBoost model (sample_weight={'yes' if sample_weight is not None else 'no'})...")
        self.model.fit(X_train, y_train, **fit_params)
        self.is_fitted = True
...
class CatBoostModel(BaseTreeModel):
    ...
    def fit(
        ...
        **kwargs
        ...
        Args:
            sample_weight: Per-observation weights. CatBoost accepts these natively
                           via Pool or direct fit parameter.
        ...
        # Build CatBoost Pool to pass sample_weight cleanly
        train_pool = cb.Pool(
            X_train, y_train,
        ...
        logger.info(f"Training CatBoost model (sample_weight={'yes' if sample_weight is not None else 'no'})...")
...
class RandomForestModel(BaseTreeModel):
    ...
    def fit(
        ...
        **kwargs
        ...
        Args:
            sample_weight: Per-observation weights passed to sklearn fit().
                           X_val / y_val accepted for API consistency but unused
                           (RandomForest has no native eval_set early stopping).
        """
        if isinstance(X_train, pd.DataFrame):
            self.feature_names = X_train.columns.tolist()
        ...
        logger.info(f"Training Random Forest model (sample_weight={'yes' if sample_weight is not None else 'no'})...")
